...
function categories_insert() {
	...
	$data = array();
	$data['CategoryName'] = $_REQUEST['CategoryName'];
	...
	if(!empty($_REQUEST['SelectedID'])) categories_copy_children($recID, $_REQUEST['SelectedID']);
...
function categories_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$templateCode = str_replace('<%%UPLOADFILE(Picture)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="Picture" id="Picture" data-filetypes="jpg|jpeg|gif|png" data-maxsize="204800">'), $templateCode);
	...
	$templateCode .= loadView('categories-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
