...
function shippers_insert() {
	...
	$data = array();
	$data['CompanyName'] = $_REQUEST['CompanyName'];
	...
	if(!empty($_REQUEST['SelectedID'])) shippers_copy_children($recID, $_REQUEST['SelectedID']);
...
function shippers_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$templateCode = str_replace('<%%UPLOADFILE(NumOrders)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		$templateCode = str_replace('<%%VALUE(NumOrders)%%>', safe_html($urow['NumOrders']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(NumOrders)%%>', urlencode($urow['NumOrders']), $templateCode);
	}else{
		...
		$templateCode = str_replace('<%%VALUE(NumOrders)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(NumOrders)%%>', urlencode(''), $templateCode);
	}
	...
	$templateCode .= loadView('shippers-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
