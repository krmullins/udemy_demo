...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(
		...
		"IF(    CHAR_LENGTH(`products1`.`UnitPrice`), CONCAT_WS('',   `products1`.`UnitPrice`), '') /* Catalog Price */" => "CatalogPrice",
		"IF(    CHAR_LENGTH(`products1`.`UnitsInStock`), CONCAT_WS('',   `products1`.`UnitsInStock`), '') /* Units In Stock */" => "UnitsInStock",
	...
	$x->SortFields = array(
		...
		2 => '`orders1`.`OrderID`',
		3 => '`products1`.`ProductName`',
		4 => 4,
		5 => '`products1`.`UnitPrice`',
		6 => '`products1`.`UnitsInStock`',
		7 => '`order_details`.`UnitPrice`',
		8 => '`order_details`.`Quantity`',
		9 => '`order_details`.`Discount`',
		10 => '`order_details`.`Subtotal`',
	);
	...
	$x->QueryFieldsFilters = array(
		...
		"IF(    CHAR_LENGTH(`categories1`.`CategoryName`) || CHAR_LENGTH(`suppliers1`.`CompanyName`), CONCAT_WS('',   `categories1`.`CategoryName`, ' / ', `suppliers1`.`CompanyName`), '') /* Category */" => "Category",
		"IF(    CHAR_LENGTH(`products1`.`UnitPrice`), CONCAT_WS('',   `products1`.`UnitPrice`), '') /* Catalog Price */" => "Catalog Price",
		"IF(    CHAR_LENGTH(`products1`.`UnitsInStock`), CONCAT_WS('',   `products1`.`UnitsInStock`), '') /* Units In Stock */" => "Units In Stock",
	...
	$x->RedirectAfterInsert = "order_details_view.php?SelectedID=#ID#";
	...
	$x->ColWidth   = array(  70, 350, 150, 75, 75, 75, 150);
	$x->ColCaption = array("Order ID", "Product", "Category", "Unit Price", "Quantity", "Discount", "Subtotal");
	$x->ColFieldName = array('OrderID', 'ProductID', 'Category', 'UnitPrice', 'Quantity', 'Discount', 'Subtotal');
	$x->ColNumber  = array(2, 3, 4, 7, 8, 9, 10);
	...
	if(strpos($x->HTML, '<!-- tv data below -->')) {
		...
		if($row = db_fetch_row($res)) {
			...
			$sumRow .= '<td class="order_details-OrderID"></td>';
			$sumRow .= '<td class="order_details-ProductID"></td>';
			$sumRow .= '<td class="order_details-Category"></td>';
			$sumRow .= '<td class="order_details-UnitPrice"></td>';
			$sumRow .= "<td class=\"order_details-Quantity text-right\">{$row[0]}</td>";
			$sumRow .= '<td class="order_details-Discount"></td>';
			$sumRow .= '<td class="order_details-Subtotal"></td>';
			$sumRow .= '</tr>';
